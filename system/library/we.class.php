...
class we
{
	...
	const TEMPLATE_GTEALLTEMLATE_URL = '/template/get_all_private_template?';
...
<Nonce><![CDATA[%s]]></Nonce>
	 ...
	 * @param array $arr
	 */
	static function json_encode(array $data){
	...
	*成功返回 当前所有消息模板
	*/
	public function getAllTemplatelists(){
		if(!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_get(self::API_URL_PREFIX.self::TEMPLATE_GTEALLTEMLATE_URL.'access_token='.$this->access_token);
		if($result){
	        $json = json_decode($result,true);
	        if (!$json || !empty($json['errcode'])) {
	            $this->errCode = $json['errcode'];
